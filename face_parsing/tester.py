...
class Tester(object):
    ...
    def test(self):
        ...
        for i in range(batch_num):
            ...
            labels_predict_plain = generate_label_plain(labels_predict, labels_predict.shape[-1])
            labels_predict_color = generate_label(labels_predict, labels_predict.shape[-1])
            for k in range(self.batch_size):
                cv2.imwrite(
                    os.path.join(self.test_label_path, str(i * self.batch_size + k) + ".png"),
